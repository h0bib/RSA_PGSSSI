...
# Module Cryptage/Decryptage

import sys, math

# The public and private keys for this program are created by the makePublicPrivateKeys.py program
# This program must be run in the same folder as the key files


# List of symbols supported in this case, you can add more symbols if you want
SYMBOLS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890 !?.,;/><=:+-_'

def encryptMsgtoSend(StringToEncrypt, mode):
    filename = 'encrypted_file.txt' # The file to write to/read from.

# the mode variable controls the behavior of the algorithm, encryption or decryption.
    if mode == 'encrypt':
        message = StringToEncrypt
        pubKeyFilename = 'ENIGMA_pubkey.txt' # the public key file 
        encryptedText = encryptAndWriteToFile(filename, pubKeyFilename, message)
        
    elif mode == 'decrypt':
        privKeyFilename = 'ENIGMA_privkey.txt' # the private key file
